...
namespace Geo.Geomagnetism
{
    public class GeomagnetismCalculator
    {
        ...
        {
            return TryCalculate(position, utcDate, null, out result);
        }
        public GeomagnetismResult TryCalculate(IPosition position, double altitude, DateTimeOffset date)
        {
            return TryCalculate(position, altitude, date.UtcDateTime);
        }

        public GeomagnetismResult TryCalculate(IPosition position, double altitude, DateTime utcDate)
        {
            GeomagnetismResult result;
            TryCalculate(position, utcDate, altitude, out result);
            return result;
        }

        public bool TryCalculate(IPosition position, DateTime utcDate, double? altitude, out GeomagnetismResult result)
        {
            var coordinate = position.GetCoordinate();
            var coordinateZ = coordinate as CoordinateZ ?? new CoordinateZ(coordinate.Latitude, coordinate.Longitude, altitude ?? 0);

            double lat = coordinateZ.Latitude.ToRadians(),
                lon = coordinateZ.Longitude.ToRadians(),
                //ele = coordinateZ.Elevation/1000,
                ele = coordinateZ.Elevation ,
